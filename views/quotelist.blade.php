...
@section('main-content')
	<div class="row">
		<div class="column col-lg-12 col-md-12 col-xs-12 col-sm-12">
				...
				<tbody>
					...
					@foreach($quotes as $quote)
						<tr>
							...
							<td>{{ $quote -> price }}</td>
							<td><a href="{{ url('viewsuplier/'.$quote -> suplierid) }}"><button type="button" class="btn btn-primary">View</button></a></td>
							@if($poststatus == 'panding')
							...
							@endif
								...
								<td>
								<a href="{{ url('../updatequote/'.$quote -> bidid.'/'.$quote -> postid) }}"><button type="button" class="btn btn-primary">Select</button></a></td>
								@else
								<td>
								<a href="{{ url('../updatequote/'.$quote -> bidid.'/'.$quote -> postid) }}"><button type="button" class="btn btn-default">Select</button></a></td>
								@endif
							@endif
						</tr>
						<?php $i++ ?>
			...
			@if (Request::path() == 'quotelist/'.$postid)

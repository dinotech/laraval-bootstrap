				...
				<tbody>
					@foreach($newpost as $post)
						<tr>
							...
							<td><a href="{{ url('sallpostdetails/'.$post -> postid) }}"><button type="button" class="btn btn-primary">View</button></a></td>
							<?php 
							date_default_timezone_set('Africa/Johannesburg');
							$currdate = date('Y-m-d H:i:s');
							?>
							@if($post -> duedate <= $currdate)
								<td><a href="{{ url('/squotelist/'.$post -> postid) }}"><button type="button" class="btn btn-primary">Quotes</button></a></td>
							@else
								<td><a><button type="button" class="btn btn-primary" disabled>Quotes</button></a></td>
							@endif
							<td><a href="{{ url('/supdatestatus/'.$post -> postid) }}"><button type="button" class="btn btn-primary">Archive</button></a></td>
						</tr>
					@endforeach
				</tbody>
				
			</table>
			@if (Request::path() == 'newposts')
				<div class="pagination">{{ $newpost -> links() }}</div>
			@endif
			</div>
		</div>
	</div>		
